...
func parseIP(fldPath *field.Path, value string, strictValidation bool) (net.IP, field.ErrorList) {
	...
	if strictValidation {
		...
		if err != nil {
			...
			allErrors = append(allErrors, field.Invalid(fldPath, value, "must not have leading 0s").WithOrigin("format=ip-strict"))
		}
		if addr.Is4In6() {
			allErrors = append(allErrors, field.Invalid(fldPath, value, "must not be an IPv4-mapped IPv6 address").WithOrigin("format=ip-strict"))
		}
	}
...
func IsValidIP(fldPath *field.Path, value string) field.ErrorList {
	...
	if value != ip.String() {
		allErrors = append(allErrors, field.Invalid(fldPath, value, fmt.Sprintf("must be in canonical form (%q)", ip.String())).WithOrigin("format=canonical"))
	}
	return allErrors
}
